...
class Admin_ProductController extends Zend_Controller_Action
{
	...
	{
		$this->view->title = Zend_Registry::get('Zend_Translate')->translate("product_add");
		...
		if ($this->_request->isPost()) {
			...
			if ($form->isValid($formData)) {
				$modelProduct = new Admin_Model_DbTable_Product();
				if ($modelProduct->add($formData)) {
	...
	{
		$this->view->title = Zend_Registry::get('Zend_Translate')->translate("products_list");
		$this->view->headTitle($this->view->title);

		$this->view->urlParams = $this->getRequest()->getUserParams();
		$this->view->paginator = Admin_Model_DbTable_Product::getPaginator($this->view->urlParams);
	}
	...
	{
		$this->view->title = Zend_Registry::get('Zend_Translate')->translate("product_edit");
		$this->view->headTitle($this->view->title);
		...
		if ($this->_request->isPost()) {
			...
			if ($form->isValid($formData)) {
				$modelProduct = new Admin_Model_DbTable_Product();
				if ($modelProduct->edit($id, $formData)) {
					$this->_flashMessenger->addMessage(array('success', 1));
					$this->_redirect('/admin/product/list');
				} else {
					$this->_flashMessenger->addMessage(array('error', 0));
			...
			$modelProduct = new Admin_Model_DbTable_Product();
			if (($data = $modelProduct->getProduct($id)) != false) {
				$form->populate($data);
			} else {
				$this->_flashMessenger->addMessage(array('error', 0));
				$this->_redirect('/admin/product/list');
	...
	{
		$this->view->title = Zend_Registry::get('Zend_Translate')->translate("product_delete");
		$this->view->headTitle($this->view->title);
