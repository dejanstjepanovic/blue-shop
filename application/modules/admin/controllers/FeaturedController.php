...
class Admin_FeaturedController extends Zend_Controller_Action
{
	...
	{
		$this->view->title = Zend_Registry::get('Zend_Translate')->translate("featured_add");
		...
		if ($this->_request->isPost()) {
			...
			if ($form->isValid($formData)) {
				$modelFeatured = new Admin_Model_DbTable_Featured();
				if ($modelFeatured->add($id, $formData)) {
					$this->_flashMessenger->addMessage(array('success', 1));
					$this->_redirect('/admin/featured/list');
				}
			} else {
    ...
    {
		$this->view->title = Zend_Registry::get('Zend_Translate')->translate("featured_list");
		$this->view->headTitle($this->view->title);

		$this->view->urlParams = $this->getRequest()->getUserParams();
		$this->view->paginator = Admin_Model_DbTable_Featured::getPaginator($this->view->urlParams);
    }

    public function editAction()
    {
		$this->view->title = Zend_Registry::get('Zend_Translate')->translate("featured_edit");
		$this->view->headTitle($this->view->title);
		...
		if ($this->_request->isPost()) {
			...
			if ($form->isValid($formData)) {
				$modelFeatured = new Admin_Model_DbTable_Featured();
				if ($modelFeatured->edit($id, $formData)) {
					$this->_flashMessenger->addMessage(array('success', 1));
					$this->_redirect('/admin/featured/list');
				} else {
					$this->_flashMessenger->addMessage(array('error', 0));
			...
			$modelFeatured = new Admin_Model_DbTable_Featured();
			if (($data = $modelFeatured->getProduct($id)) != false) {
				$form->populate($data);
			} else {
				$this->_flashMessenger->addMessage(array('error', 0));
				$this->_redirect('/admin/featured/list');
    ...
    {
		$this->view->title = Zend_Registry::get('Zend_Translate')->translate("featured_delete");
		$this->view->headTitle($this->view->title);
