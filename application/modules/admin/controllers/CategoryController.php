...
class Admin_CategoryController extends Zend_Controller_Action
{
	...
	{
		$this->view->title = Zend_Registry::get('Zend_Translate')->translate("category_add");
		...
		if ($this->_request->isPost()) {
			...
			if ($form->isValid($formData)) {
				$modelCategory = new Admin_Model_DbTable_Category();
				if ($modelCategory->add($formData)) {
	...
	{
		$this->view->title = Zend_Registry::get('Zend_Translate')->translate("categories_list");
		$this->view->headTitle($this->view->title);

		$this->view->urlParams = $this->getRequest()->getUserParams();
		$this->view->paginator = Admin_Model_DbTable_Category::getPaginator($this->view->urlParams);
	}

	public function editAction()
	{
		$this->view->title = Zend_Registry::get('Zend_Translate')->translate("category_edit");
		$this->view->headTitle($this->view->title);
		...
		if ($this->_request->isPost()) {
			...
			if ($form->isValid($formData)) {
				$modelCategory = new Admin_Model_DbTable_Category();
				if ($modelCategory->edit($id, $formData)) {
					$this->_flashMessenger->addMessage(array('success', 1));
					$this->_redirect('/admin/category/list');
				} else {
					$this->_flashMessenger->addMessage(array('error', 0));
			...
			$modelCategory = new Admin_Model_DbTable_Category();
			if (($data = $modelCategory->getCategory($id)) != false) {
				$form->populate($data);
			} else {
				$this->_flashMessenger->addMessage(array('error', 0));
				$this->_redirect('/admin/category/list');
	...
	{
		$this->view->title = Zend_Registry::get('Zend_Translate')->translate("category_delete");
		$this->view->headTitle($this->view->title);
